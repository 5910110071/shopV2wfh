        ...
        <div className="card mb-4 ">
          ...
          <div className="row d-flex justify-content-center">
            {basket.orders && basket.orders.map((record) => {
              return (
                <div key={record.product.product_id} className="col-5 d-flex flex-column bd-highlight mb-2">
                  ...
                  <div className="container input-group d-flex justify-content-center   ">
                    <span class="input-group-btn ">
                      <button type="button" class="quantity-left-minus btn btn-secondary btn-number" data-type="minus" data-field="" onClick={() => { this.props.basketEditQuantity(record.product.product_id, this.props.user.id, "-") }}>
                        <span class="glyphicon glyphicon-minus">-</span>
                      </button>
                    </span>
                    <input type="text" id="quantity" name="quantity" class="form-control input-number col-1 text-center" value={record.quantity} min="1" max="10" />
                    <span class="input-group-btn">
                      <button type="button" class="quantity-right-plus btn btn-secondary btn-number mr-2" data-type="plus" data-field="" onClick={() => { this.props.basketEditQuantity(record.product.product_id, this.props.user.id, "+",record.quantity,record.product.product_inventory) }}>
                  ...
                  <div className="d-flex justify-content-center">
                    <button className="btn btn-danger mb-2 " onClick={() => this.props.basketDeleteProduct(basket._id, record.product.product_id, this.props.user.id)}>X</button>
                  </div>
          ...
          <div className="d-flex justify-content-center">
            <button className="btn btn-secondary title mr-2 mb-3" onClick={() => onSubmit(basket._id)}>ยกเลิกรายการ</button>
            <button className="btn btn-danger title mr-2 mb-3" onClick={() => this.props.history.push("/paymentNotification/" + basket.user_id)}>แจ้งชำระเงิน</button>
          </div>
        </div>
      </div>
    );
